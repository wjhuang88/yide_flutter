...
class _RepeatTile extends StatefulWidget {
  final bool selected;
  ...
  const _RepeatTile({Key key, this.selected = false, this.title, this.onTap})
      : _extended = false,
        extend = null,
        super(key: key);
  const _RepeatTile.extended(
      {Key key,
      this.selected = false,
  ...
  @override
  _RepeatTileState createState() => _RepeatTileState();
}

class _RepeatTileState extends State<_RepeatTile>
  ...
  @override
  void didUpdateWidget(_RepeatTile oldWidget) {
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      child: Transform.scale(
        scale: 1.0 - 0.2 * Math.sin(_transValue * Math.pi),
        child: tile,
      ),
      onTap: widget.onTap ?? () {},
    );
